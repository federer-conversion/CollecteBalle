...
class ImageParser(Node):
    ...
    def __init__(self):
        ...
        self.robot_publisher = self.create_publisher(PoseStamped, 'robot_position', 10)
    ...
    def detect_ball_in_image(self):
        ...
        image_robot = cv2.inRange(image_HSV, self.robot_low_HSV, self.robot_high_HSV)
        image_pince = cv2.inRange(image_HSV, self.pince_low_HSV, self.pince_high_HSV)
        ...
        cnts = cv2.findContours(image_robot, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cnts = cnts[0] if len(cnts) == 2 else cnts[1]
        for c in cnts:
            x,y,w,h = cv2.boundingRect(c)
            if(w>10 and h>15):
        ...
        cnts = cv2.findContours(image_pince, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cnts = cnts[0] if len(cnts) == 2 else cnts[1]
        for c in cnts:
            x,y,w,h = cv2.boundingRect(c)
            if(w>20 and h>20):
